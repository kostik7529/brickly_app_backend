...
public class ThemeController {
    ...
    public ResponseEntity<Page<ThemeDefaultDTO>> getRootThemesPaginated(@RequestParam(defaultValue = "0")int page,@RequestParam(defaultValue = "10")int size){
        Pageable pageable = PageRequest.of(page,size);
        return  ResponseEntity.ok(themeService.getRootThemesPaginate(pageable));
    }

    @GetMapping("/by_parent_id/{parentId")
    public ResponseEntity<Page<ThemeDefaultDTO>> getChildThemesByParentId(@PathVariable Integer parentId,@RequestParam(defaultValue = "0")int page,@RequestParam(defaultValue = "10")int size){
        Pageable pageable = PageRequest.of(page,size);
        return ResponseEntity.ok(themeService.getChildThemesByParentId(parentId, pageable));
    }
